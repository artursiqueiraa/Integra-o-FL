...
| `OperationController.cs` | `api/Operation/enviar`, `api/Operation/historico` | `PgmService` diretamente (mesmo serviço da Tela Central — sem simulação, ver seção 6) |
| `PgmPredioController.cs` | `api/PgmPredio` (CRUD do cadastro de PGMs por Central) | `PgmPredioService` |
| `ZonaPredioController.cs` | `api/ZonaPredio` (CRUD do cadastro de Zonas por Central) | `ZonaPredioService` |
...
| `PgmPredioService.cs` | `PgmPredioService` | CRUD do cadastro de PGMs (nome/tipo/ícone) de uma Central, usado pela tela Operação | `AppDbContext` |
| `ZonaPredioService.cs` | `ZonaPredioService` | CRUD do cadastro de Zonas (nome/tipo) de uma Central, usado pela tela Operação | `AppDbContext` |

> `ConnectionService.cs` e o endpoint `POST /api/central/testar-conexao` **foram removidos**
> (não apenas marcados como legado) numa limpeza anterior — ver seção 6 e
> [`ARQUITETURA_SESSION_MANAGER.md`](ARQUITETURA_SESSION_MANAGER.md). `OperationService.cs`
> (envio de PGM simulado via `AdapterFactory`/`FakeAdapter`) **também foi removido** depois —
> `OperationController` hoje chama `PgmService` diretamente, o mesmo serviço real da Tela
> Central. `AdapterFactory`/`JflAdapter`/`IntelbrasAdapter`/`FakeAdapter` (SDK, seção 6) ficaram
> sem nenhum consumidor real como consequência.
...
`CentralStatusDtos.cs` (todo o retorno de `/status`), `OperationDtos.cs` (`ComandoDto`/
`HistoryDto`, ainda usados por `OperationController`, agora contra o fluxo real), `PgmDtos.cs`,
`ArmDtos.cs`, `ZoneInhibitDtos.cs`, `PgmPredioDtos.cs`, `ZonaPredioDtos.cs`. DTOs existem para
**nunca expor as entidades do banco
...
`Building.cs`, `Central.cs`, `CentralSession.cs`, `History.cs`, `PgmPredio.cs`, `ZonaPredio.cs` —
detalhados campo a campo em [`06_DATABASE_GUIDE.md`](06_DATABASE_GUIDE.md).
...
│   └── OperationPage.tsx          → painel dinâmico por Prédio/Central — mesmos serviços reais da Tela Central
└── components/
    ├── PgmPanel.tsx               → painel reutilizável dos 16 botões de PGM (aceita `catalogo` opcional de PgmPredio)
    ├── ArmPanel.tsx               → painel de Arme: 16 partições (Armar/Desarmar/Stay/Away) + card do eletrificador
    ├── ZonasPanel.tsx             → chips de zona clicáveis (aceita `catalogo` opcional de ZonaPredio)
    ├── CadastroPgmZonaPanel.tsx   → CRUD inline de PgmPredio/ZonaPredio, usado pela tela Operação
...
um projeto separado `Backend/CentralHub.Api.Tests` (40 testes, cobrindo `SessionActivityLogService`,
`CentralSessionService`, `ArmService`, `ZoneInhibitService`, `PgmPredioService` e `ZonaPredioService`
— as duas últimas com EF Core InMemory puro; as demais contra um `JflTcpServer` real e o Central
Simulator, sem mocks) no momento em que este documento foi escrito — ver
...
> `OperationService.cs` (envio de PGM simulado, base da tela "Operação" original) **foi removido**
> — `OperationController` hoje injeta `PgmService` diretamente (o mesmo serviço real usado pela
> Tela Central) e `Frontend/.../OperationPage.tsx` foi reescrita como um painel dinâmico
> (Prédio → Central) que reaproveita `ArmPanel`/`PgmPanel`/`ZonasPanel` — ver
> [`09_WEB_GUIDE.md`](09_WEB_GUIDE.md), seção 6.
